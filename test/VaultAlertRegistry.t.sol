...
contract VaultAlertRegistryTest is Test {
    VaultAlertRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_lido_vaultmonitor");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new VaultAlertRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        VaultAlertRegistry freshController = new VaultAlertRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
